...
local fonts = {}; do
    ...
    local ProggyTiny = Register_Font("ProggyClean", 200, "Normal", {
        Id = "ProggyClean.ttf",
        Font = game:HttpGet("https://raw.githubusercontent.com/0x340/Priv9-UI-Libary/main/fonts/ProggyClean.ttf"),
    })

    fonts = {
        ["ProggyClean"] = Font.new(ProggyTiny, Enum.FontWeight.Regular, Enum.FontStyle.Normal);
    }
end
-- 
    ...
    for name, value in pairs(flags) do
        if type(value) == "table" and value.key then
            Config[name] = {
                active = value.active, 
                mode = value.mode, 
                key = tostring(value.key):gsub("Enum.KeyCode.", ""):gsub("Enum.UserInputType.", "")
            }
...
function library:load_config(config_json) 
    ...
    for name, value in pairs(config) do 
        local function_set = config_flags[name]
        
        if name == "config_name_list" then 
            continue
        end
        ...
        if function_set then 
            ...
            elseif type(value) == "table" and value["active"] then 
                local key_enum
                if value.key ~= "..." then
                    if Enum.KeyCode[value.key] then
                        key_enum = Enum.KeyCode[value.key]
                    elseif Enum.UserInputType[value.key] then
                        key_enum = Enum.UserInputType[value.key]
                    else
                        key_enum = "..."
                    end
                else
    ...
    for _, instance in path:GetDescendants() do 
        if not instance:IsA("GuiObject") then 
            if instance:IsA("UIStroke") then
                tween_service:Create(instance, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Transparency = fading}):Play()
            end
...
function notifications:create_notification(options)
    ...
    tween_service:Create(outline, TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Position = dim_offset(50, 50 + ((index-1) * 30))}):Play()
...
function library:watermark(options)
    ...
    -- Instances
        local outline = library:create("Frame", {
            ...
            AutomaticSize = Enum.AutomaticSize.X;
            BackgroundColor3 = themes.preset.accent
        }); library:apply_theme(outline, "accent", "BackgroundColor3"); library:draggify(outline);
...
local watermark = library:watermark({name = "priv9"})
local fps = 0
...
run.RenderStepped:Connect(function()
    ...
    if tick() - watermark_delay > 1 then 
        ...
        local current_fps = math.floor(1 / run.RenderStepped:Wait())
        watermark.update_text(string.format("priv9 | fps: %s | ping: %s", current_fps, ping))
        fps = 0
    end
end)
...
function library:list(options)
    local cfg = {
        ...
        flag = options.flag or "list_flag", 
    }
...
function library:slider(options) 
    ...
    -- Instances 
        ...
        local eeeee = library:create("TextLabel", {
            ...
            RichText = true;
            BorderColor3 = rgb(0, 0, 0);
            Text = cfg.name .. "<font color='#AAAAAA'> - " .. tostring(cfg.default) .. cfg.suffix .. "</font>";
            Parent = slider;
            Size = dim2(1, 0, 0, 0);
...
function library:dropdown(options) 
    ...
    -- Instances
        -- Element 
            ...
            local text = library:create("TextLabel", {
                ...
                TextColor3 = themes.preset.text;
                BorderColor3 = rgb(0, 0, 0);
                Text = cfg.multi and "Select..." or cfg.items[1];
                Parent = inline;
                Size = dim2(1, 0, 1, 0);
                BackgroundTransparency = 1;
        ...
        -- Holder
            local accent = library:create("Frame", {
                Parent = library.gui;
                Size = dim2(0, dropdown_holder.AbsoluteSize.X, 0, 0);
    ...
    -- Connections 
        dropdown_holder.MouseButton1Click:Connect(function()
            ...
            accent.Size = dim2(0, dropdown_holder.AbsoluteSize.X, 0, 0)
...
function library:colorpicker(options) 
    local cfg = {
        ...
        color = options.color or rgb(1, 1, 1),
...
function library:keybind(options) 
    local cfg = {
        ...
        key = options.key or Enum.KeyCode.LeftShift, 
    ...
    -- Instances
        ...
        -- Holder
            local accent = library:create("Frame", {
                Parent = library.gui;
                Visible = false;
                Size = dim2(0, keybind_holder.AbsoluteSize.X, 0, 0);
                Position = dim2(0, 500, 0, 100);
                BorderColor3 = rgb(0, 0, 0);
                BorderSizePixel = 0;
                AutomaticSize = Enum.AutomaticSize.Y;
    ...
    -- Functions 
        function cfg.modify_mode_color(path)
            ...
            if cfg.hold_instances[path] then 
                cfg.hold_instances[path].TextColor3 = themes.preset.text
            end
        end 
        ...
        function cfg.set(input)
            ...
            elseif typeof(input) == "EnumItem" then 
                input = input == Enum.KeyCode.Escape and "..." or input
            ...
            elseif type(input) == "table" then 
                if input.key then
                    if type(input.key) == "string" and input.key ~= "..." then
                        if Enum.KeyCode[input.key] then
                            input.key = Enum.KeyCode[input.key]
                        elseif Enum.UserInputType[input.key] then
                            input.key = Enum.UserInputType[input.key]
                        end
                    end
        ...
        function cfg.set_visible(bool)
            ...
            accent.Size = dim2(0, keybind_holder.AbsoluteSize.X, 0, 0)
